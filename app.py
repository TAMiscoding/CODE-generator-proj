...
# -----------------------------
st.set_page_config(
    page_title="Constraint Code Generator",
    page_icon="🧠",
    layout="wide"
)

st.title("🧠 Constraint-Driven Code Generator")
st.caption("Generates ONLY code based on strict instructions")
    ...
    with st.spinner("Generating code..."):
        try:
            ...
            if "429" in msg or "RESOURCE_EXHAUSTED" in msg:
                st.warning(
                    "🚫 **API quota exceeded**\n\n"
                    "You are using the Gemini free tier.\n"
                    "Please wait about a minute or try again later.\n\n"
                    "👉 Tip: Avoid repeated rapid clicks during testing."
                )
                st.stop()
            else:
                st.error(msg)
...
if st.session_state.last_output:
    ...
    st.code(
        st.session_state.last_output["final_prompt"],
        language="text"
    )
